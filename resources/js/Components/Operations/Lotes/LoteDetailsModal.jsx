...
export default function LoteDetailsModal({ show, onClose, lote, proyecto, inventoryItems, stages, members, initialIsFinishing = false }) {
    ...
    return (
        <Modal show={show} onClose={onClose} maxWidth={isFinishing ? 'xl' : '3xl'}>
            {isFinishing ? (
                <div className="p-6">
                    <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">{t('operations.finish_modal_title', 'Finalizar Lote')}</h2>
                    ...
                    <form onSubmit={handleFinish} className="space-y-4">
                        ...
                        <div className="flex justify-end gap-2 mt-6">
                            <SecondaryButton onClick={() => setIsFinishing(false)} disabled={processingFinish}>
                                {t('common.cancel', 'Cancelar')}
                            </SecondaryButton>
                            <PrimaryButton disabled={processingFinish}>
                ...
                <div className="p-6">
                    ...
                    <Tab.Group>
                        ...
                        <Tab.Panels>
                                    ...
                                    <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                                        ...
                                        <p className="text-xs text-gray-500">
                                            {t('operations.cost_total', 'Costo consumido')}: {formatCurrency(lote.inputs?.filter(i => i.status === 'consumed').reduce((acc, curr) => {
                                                // Robust calculation: prefer total_cost from DB, but if 0/null/inconsistent, recalc from unit price (if available)
                                                let totalCost = Number(curr.total_cost);
                                                // Fallback if total_cost is 0 or invalid, try to calc from unit price
                            ...
                            {/* INPUTS TAB */}
                            <Tab.Panel>
                                {/* Pending Inputs Section Removed as per request to avoid double consumption */}
                                {/* 
                            <div className="mb-6">
                                                    ...
                                                    {lote.inputs.filter(i => i.status === 'consumed').map((input) => (
                                                        <tr key={input.id}>
                                                            ...
                                                            <td className="px-4 py-2 text-sm">
                                                                {/* Display Saved Total OR Calculated if missing */}
