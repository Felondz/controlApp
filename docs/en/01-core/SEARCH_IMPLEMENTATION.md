...
## Overview
ControlApp uses **Meilisearch** via **Laravel Scout** to provide fast, relevant, and secure search capabilities. The search functionality is integrated globally in the topbar and supports indexing of Users and Projects.

## Architecture

### Backend
- **Driver**: `meilisearch` (Production/Dev), `collection` (Testing).
...
### Frontend
- **Component**: `SearchInput.jsx` (Topbar widget).
...
- **Members**: Users can only find projects they are a member of.
...
- **Logic**:
  ...
  $proyectos = Proyecto::search($query)->get()->filter(function ($proyecto) use ($user) {
      // Only return projects where the user is the owner or an admin
...
### Prerequisites
- Meilisearch instance running (e.g., via Docker).
