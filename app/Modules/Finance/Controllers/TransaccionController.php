...
class TransaccionController extends Controller
{
    ...
    public function store(StoreTransaccionRequest $request, Proyecto $proyecto)
    {
        ...
        app(ModuleEventBus::class)->dispatch(
            new TransactionCreated($transaccion)
        );
    ...
    public function update(UpdateTransaccionRequest $request, Proyecto $proyecto, Transaccion $transaccion)
    {
        ...
        $datosValidados = $request->validated();
        $changes = $transaccion->getDirty();
        $transaccion->update($datosValidados);

        // Dispatch event
        app(ModuleEventBus::class)->dispatch(
            new \App\Modules\Finance\Events\TransactionUpdated($transaccion, $changes)
        );
    ...
    public function destroy(Request $request, Proyecto $proyecto, Transaccion $transaccion)
    {
        ...
        $accountId = $transaccion->cuenta_id;
        $projectId = $transaccion->proyecto_id;

        $transaccion->delete();

        // Dispatch event
        app(ModuleEventBus::class)->dispatch(
            new \App\Modules\Finance\Events\TransactionDeleted($transactionId, $projectId, $accountId)
        );
